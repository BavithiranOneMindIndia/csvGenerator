...
def render_ui():
    ...
    if uploaded_file:
        ...
        for col in df.columns:
            col_type = st.selectbox(f"Select type for **{col}**", 
                                    ["Same Value", "Random", "Number Range", "Date", "Text"], 
                                    key=f"type_{col}")
            column_types[col] = col_type

            # UI options based on column type
        ...
        if st.button("Generate Data"):
            ...
            st.download_button("Download CSV", data=csv_data, file_name="generated_data.csv", mime="text/csv")
